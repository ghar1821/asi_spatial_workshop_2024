...
using 10x software spaceranger (https://www.10xgenomics.com/support/software/space-ranger/latest).
Running spaceranger can be very complex (depending on your experiment), and 
it is beyond the scope of this workshop. 

If you are keen to try running spaceranger, here is the command you can use for this dataset:
...
Note, make sure you download all the input files from 10x website and the
relevant reference genome also from their website.
...
Demo on how to use Loupe browser to select spots to remove:

<video width="600" height="500" controls>
  <source src="assets/visium_loupe_demo.mp4" type="video/mp4">
</video>

After saving the spots barcode in a csv file, we can load it up and remove them from the seurat object:

```{r}
spots_to_remove <- read.csv("data/visium_human_crc/spots_to_remove.csv")
...
dat_fil_loupe <- subset(x = dat_fil, remove_spot_from_loupe == FALSE)
dat_fil_loupe
```


#### Using interactive SpatialDimPlot
...
Let's interactively visualise the tissue and the spot, and hover over each spot 
that lies outside of the tissue to note their ID.
...
Store the ID somewhere safe and paste them in as a vector below so we can remove them.
...
It is slightly different from *highly variable genes* commonly identified in
single cell data where we find genes which expressions show high variability across cells or samples.
It adds an extra spatial dimension to the analysis in that its purpose is to find genes 
which expression level not only vary but also show pattern of variability that is 
dependent on their location in the tissue.
Specifically, it means, SVGs' expression is not randomly distributed, but rather, 
correlated with the spatial structure of the tissue.
It can show the biological processes that are happening in different part of the tissue.

```{r}
# require HVGs to already be detected.
# the function below takes a while to run. consider reducing the number of HVGs
dat_fil <- FindSpatiallyVariableFeatures(
  dat_fil,
  assay = "SCT",
  features = VariableFeatures(dat_fil)[1:500],
  selection.method = 'moransi'
)
```

Visualise top 10 SVGs:

```{r}
# a bit of data wrangling to get SVGs
svgs_meta <- dat_fil[['SCT']][[]]
...
Compare this with non-SVGs (TMEM200B and CLEC10A are HVGs but not SVGs):

```{r}
# TMEM200B, CLEC10A are HVGs but not SVGs
# SYNM, MGP are HVGs
SpatialFeaturePlot(
  dat_fil,
...
We can inspect SVGs against the module scoring to see if there are relationship between the 
different cell types and the genes:

```{r}
SpatialFeaturePlot(
  dat_fil,
  features = c(
    head(rownames(svgs_meta), 9), "malignant_score"
  ),
  ncol = 5,
  image.alpha = 0.3,
  pt.size.factor = pt_size
...
We can see CD24 is highly correlated with the module score for tumour cells, that
are often associated with cancer stemness, immune evasion, metastasis, and tumour progression.
GPX2 also show some correlation with tumour module score that promotes tumour growth and metastasis.
CXCL2 and CXCL3 also highly expressed in tumour modules. 
The latter promotes malignant behaviour in tumour cells.

We can also grab top few hundred genes and run pathway analysis to see which biological
...
The bar above was generated by Metascape to show the some biological pathways associated
with top 500 SVGs we found.
From example, ECM organisation NABA core matrisome may suggest some changes in ECM structure and
composition that may be crucial in tumour progression or metastasis.
Epithelial Cell Differentiation may mean there are changes in the normal differentiation 
process of epithelial cells in the colon.

## Extra stuff

Deconvolution - if there is a nice single cell data we can use.
