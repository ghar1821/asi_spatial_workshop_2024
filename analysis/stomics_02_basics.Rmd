...
ggplot(polygons) +
  ...
  theme_void()
```
Now I'm just going to clean these up slightly (the polygons aren't closed).
...
In case that didn't work, here's an example of generating a polygonal region.
...
which allows us to make use of these geometries.
...
Sometimes we might want to extract those regions out into separate objects, which
we can do with the `crop` function:
