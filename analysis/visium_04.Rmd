...
In this part of the workshop, we will learn how to use module scoring and
...
```{r}
sc_data <- qread("data/single_cell/seurat_object_10x.qs")
...
# qsave the object
qsave(RCTD, "output/visium/rctd_decon_out.qs")
```
...
We can then attach it to our Seurat object and interrogate it.
...
We can assign labels to the spots if the majority of the cell types (>80%) in the spot
are made of one cell type:

```{r}
dat[[]]$majority_decon_per_spot <- apply(norm_weights, 1, function(x) {
    # Find the maximum value in the row
    max_prop <- max(x)
    if (max_prop > 0.8) {
      return(gsub("decon_", "", names(which.max(x))))
    } else {
      return("mix")
    }
    
})
DimPlot(dat, group.by = "majority_decon_per_spot")
SpatialDimPlot(dat, group.by = "majority_decon_per_spot", pt.size.factor = 5, image.alpha = 0.5)
```


## Cross checking module scoring and deconvolution
...
Comparing the two, we can see areas where concordance we get some concordance,
...
We can focus on one difference (say the tumour vs epithelial region) and investigate further.
